...
DEFAULT_TEMPLATE='php7.0-nginx-mysql5.7';
...
if [ "$#" -lt 6 ]; then
   echo "${red}Invalid number of parameters. You should pass \$domain, \$prefix, \$webPort, \$dbPort, \$sshPort, \$xdebugPort and optionally \$templateName${reset}";
   return;
fi
...
sed -i -e "s/\${dbPort}/$4/g" ${OUTPUT_DIR}docker-compose.yml;
sed -i -e "s/\${sshPort}/$5/g" ${OUTPUT_DIR}docker-compose.yml;
...
sed -i -e "s/\${xdebugPort}/$6/g" ${OUTPUT_DIR}php/config/conf.d/20-xdebug.ini;
